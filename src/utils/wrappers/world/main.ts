...
export default class World {
  ...
  public static GetMoneyObjective(): server.ScoreboardObjective {
    const manager = server.world.scoreboard;

    return (
      manager.getObjective(Config.MoneyObjective) ??
      manager.addObjective(Config.MoneyObjective)
    );
  }
  public static GetMoney(member: Member): number {
    const objective = this.GetMoneyObjective();
    let score = 0;

    try {
      score = objective.getScore(member.Player()) ?? 0;
    } catch {}

    return score;
  }
  public static SetMoney(member: Member, score: number): void {
    const objective = this.GetMoneyObjective();

    objective.setScore(member.Player(), score);
  }
}
